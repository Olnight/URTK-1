...
class ShopProduct
{
    ...
    public function actionView($id, PDO $pdo): void 
    {
    ...
    public function actionUpdate($id, PDO $pdo): void 
    {
    ...
    public function actionDelete($id, PDO $pdo): void
    {
    ...
    protected function find($id, PDO $pdo): void 
    {
        $stmt = $pdo->prepare("SELECT * FROM products WHERE id = ?");
        $stmt->execute([$id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
